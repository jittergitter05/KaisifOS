    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 hover:border-slate-700 transition-colors shadow-sm">
      ...
      <div className="space-y-4 pt-2">
          ...
          <div className="bg-slate-800/50 p-4 rounded-md border border-slate-700/50">
             <h4 className="text-[10px] font-bold tracking-wider text-emerald-500 uppercase mb-2">DM Draft</h4>
             <p className="text-sm italic text-slate-400">{job.DM}</p>
          </div>
          <div>
             <h4 className="text-[10px] font-bold tracking-wider text-slate-500 uppercase mb-1">Resume Angle</h4>
             <p className="text-sm text-slate-300">{job.ResumeAngle}</p>
          </div>
      </div>
      
      <div className="mt-6 pt-4 border-t border-slate-800 flex justify-between items-center">
         <span className="text-xs font-mono text-slate-500">Found {job.Date}</span>
         <a href={job.URL} target="_blank" rel="noreferrer" className="inline-flex items-center px-4 py-2 border border-emerald-500/20 text-xs font-medium rounded-md text-emerald-400 hover:bg-emerald-500/10 focus:outline-none transition-colors">
            Apply Now
         </a>
      </div>
    </div>
